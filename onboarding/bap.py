...
def search():
    ...
    search_req_str = json.dumps(search_req)
    headers = {
        "Content-Type": "application/json",
        "Authorization": create_authorisation_header(search_req_str)
    }
    start_time = time.time()
    response = requests.post(f"{gateway_url}/search", data=search_req_str, headers=headers)
    duration = time.time() - start_time
    outgoing_request_histogram.labels(host=f"{gateway_url}", endpoint="/search", method="POST", status=response.status_code).observe(duration)
    ...
    if response.status_code == 200:
        return { "message" : f"/search passed for transaction_id : {search_req['context']['transaction_id']}" }
    else:
        return { "message" : f"/search failed for transaction_id : {search_req['context']['transaction_id']} | {response.text}" }
...
def on_search():
    on_search_req = request.get_json()
    print({ "pod" : os.getenv("POD_NAME"), "action" : "on_search", "transactionId" : on_search_req['context']['transaction_id'], "messageId" : on_search_req['context']['message_id'] })
    return { "message" : f"/on_search recieved for transaction_id : {on_search_req['context']['transaction_id']} | message_id : {on_search_req['context']['message_id']}" }
